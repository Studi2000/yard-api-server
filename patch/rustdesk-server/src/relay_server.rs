...
fn post_session_event(event: &str, uuid: &str, viewer_ip: &str, target_ip: &str, target_id: &str) {
    // Try to read the API URL from environment variable
    let api_url = match env::var("YARD_API_URL") {
        Ok(url) => url,
        Err(_) => {
            log::info!("YARD_API_URL not set in environment, not sending API session event!");
            return;
        }
    };
    ...
    match res {
        Ok(_) => log::info!("Sending Session-Event {} OK!", event),
        Err(e) => log::info!("Sending Session-Event FAILED! {:?}", e),
    }
}
                ...
                if !rf.uuid.is_empty() {
                    ...
                    if let Some(peer) = peer.as_mut() {
                        ...
                        let target_ip = peer
                            .as_ref()
                            .as_any()
                            .downcast_ref::<FramedStream>()
                            .map(|fs| fs.local_addr().ip().to_string())
                            .unwrap_or_else(|| "".to_string());

                        log::info!("Relayrequest {} from {} got paired (target_ip={}, target_id={})", rf.uuid, addr, target_ip, rf.id);

                        post_session_event("start", &rf.uuid, &addr.to_string(), &target_ip, &rf.id);
                        ...
                        if let Err(err) = relay(addr, &mut stream, peer, limiter, id.clone()).await
                        {
                            ...
                            post_session_event("end", &rf.uuid, &addr.to_string(), &target_ip, &rf.id);
                        } else {
